...
import {
  useCreateFacebookMutation,
  useDeleteFacebookMutation,
...
const Facebook = ({ uid }) => {
  ...
  const [createFacebookLink] = useCreateFacebookMutation();
  const { data: singleUser } = useGetUserByUidQuery(uid);

  const onSubmit = async (data) => {
    try {
      setLoading(true);
      const result = await createFacebookLink({ uid: uid, data: data });
  ...
  const [deleteFacebook] = useDeleteFacebookMutation();
  ...
  const handleDelete = async (_id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const result = await deleteFacebook({ uid: uid, id: _id });
          if (result.data.message === "Facebook link deleted successfully") {
    ...
    <div className="flex flex-col md:flex-row items-start gap-4">
          ...
          <div className="w-full flex flex-col gap-2 items-start">
            <div className="w-full">
              <div className="flex items-center gap-2 mb-1 w-full">
                <div className="form-control border-0 p-0 bg-transparent w-full">
                  <input
                    type="text"
                    name="facebook"
                    {...register("facebook", {
                    ...
                    className="p-1 border-2 border-solid border-yellow-400 rounded-none outline-none placeholder:text-gray-500 w-full bg-white"
                  />
                  {errors.facebook && (
      ...
      <div className="border-2 border-gray-500 w-full rounded-md p-3 flex flex-col items-start gap-4">
        ...
        <div className="w-full flex flex-col gap-2">
          {singleUser &&
            singleUser?.facebook?.map((fb, index) => (
              <div
                key={index}
                  ...
                  <div className="">
                    <Link to={fb?.facebook?.facebook}>
                      {fb?.facebook?.facebook?.length > 35 ? (
                        <>
                          {fb?.facebook?.facebook?.slice(0, 35)}
                          ...
                        </>
                      ) : (
